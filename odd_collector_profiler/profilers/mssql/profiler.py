...
from odd_collector_profiler.domain.config import MSSqlConfig
...
class MssqlProfiler(Profiler, SQLDialect):
...
def register_profiler(config: Dict[str, Any]) -> Profiler:
    return MssqlProfiler(config=config, data_profiler=DATA_PROFILER)
